...
| `esp32_probe_i0_populated.derived.json` | **derived** | Multi-sensor shape with `fault` and `open` channels, from `web_server.h:34-46` + `payload.h:31-35`. Kept because no real probe has been faulted yet; replace once one is. |
